...
consumidor analítico. Ela precisa, no mínimo, satisfazer a
Consideração do PDF do case:

> "Você precisa garantir que as colunas `VendorID`, `passenger_count`,
...
do case (a média mensal de `total_amount` em yellow, e a média de
...
- **Tráfego de consumo**: case avaliativo — as queries das duas
  perguntas rodam algumas vezes durante revisão, não centenas de
  vezes ao dia.
- **Governança**: leitor único (o avaliador). Não há múltiplas equipes
...
- **Heterogeneidade de schema**: yellow publica `tpep_pickup_datetime` /
  ...
  os nomes nativos no silver). O nome `tpep_*` exigido pelo case
  precisa ser exposto na gold para corridas green também.
...
pré-agregados** que respondam diretamente cada pergunta do case, ou
...
COMMENT 'Gold: view de consumo unificada (yellow+green) com as colunas exigidas pelo case.'
AS
...
- **Alias `lpep_*` -> `tpep_*` na metade green do UNION**: normaliza o
  nome ao contrato do case sem perder a linhagem (`taxi_type` literal
  preserva a origem por linha).
- **Sem `SET TAGS`**: `ALTER VIEW SET TAGS` não é uniforme entre
  runtimes; metadados de governança ficam no `COMMENT`.

**As perguntas analíticas do case são respondidas por SQL ad-hoc**
contra essa view, vivendo em `analysis/perguntas_case.sql`. Não há
...
**Nenhuma das três condições existe no escopo do case**: 18M linhas
agregam em segundos; o tráfego de consumo é o avaliador rodando a
query algumas vezes; não há requisito regulatório. Materializar
fatos aqui adicionaria tabelas Delta, código de `ensure_*`, `build_*`
e `overwrite`, refresh adicional a cada execução do job — sem
contrapartida em performance, em correção ou em valor de negócio.

Também há um argumento de fidelidade ao enunciado: o case pede
explicitamente "Código SQL ou PySpark estruturado **da forma que
preferir** com as respostas para as seguintes perguntas". Resposta =
query; o caminho mais direto é colocar a query em `analysis/` rodando
contra a view, não pré-materializar a resposta dentro de uma tabela.
...
- **Contrato do case atendido no nível certo da pilha**: as 5 colunas
  exigidas estão na *camada de consumo* (gold), não enterradas na
  silver (que mantém os nomes nativos do TLC, `lpep_*` inclusive).
...
- **Coerente com a diretriz do projeto** ("evitar over-engineering")
  e com o que o case literalmente pede (SQL com a resposta).
...
- **A definição da métrica vive na query SQL, não na schema da
  ...
  Mitigação: as queries vivem em `analysis/perguntas_case.sql`,
...
2. **Tráfego**: a query roda algumas vezes durante revisão. O custo
   ...
   todas as execuções ad-hoc projetadas no horizonte do case.
...
Especulação prematura. O case tem 2 perguntas; modelo dimensional
paga dividendo a partir de ~10 queries recorrentes. Fazer agora seria
exatamente o tipo de over-engineering que o projeto pede para
evitar.
...
Falharia a Consideração explícita do PDF: as colunas exigidas
(`tpep_pickup_datetime`, `tpep_dropoff_datetime`) não existem em
green-silver — lá são `lpep_*` (ADR-013 preserva os nomes nativos do
TLC no silver, por boas razões). A gold é onde esse contrato com o
case se realiza. Sem a view, o consumidor precisa conhecer a
heterogeneidade do TLC; com a view, o contrato é uniforme.
...
- **Schema fidelity**: `DESCRIBE EXTENDED <catalog>.gold.vw_taxi_trips`
  ...
  (`_CASE_REQUIRED_COLUMNS` + `taxi_type`). Drift = PR explícito.
- **Sanity numérico**: as duas queries de Pergunta A e B em
  `analysis/perguntas_case.sql` devem rodar sem erro e produzir
...
Pergunta correspondente em `analysis/perguntas_case.sql` vira um
fato pré-agregado (`CREATE TABLE AS SELECT` + agendamento mensal),
